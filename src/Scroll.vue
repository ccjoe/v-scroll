    ...
    <div v-bind:class="'ui-scroll ui-scroll-'+(isx?'x':'y')" v-bind:style="uiScrollStyle">
        <div v-bind:class="'ui-scroll-content '+ (isEnd || !enable ? 'ui-scroll-trans' : '')" v-bind:style="scrollContentStyle">
            <slot name="top" v-if="top">
                <div class="ui-scroll-top ui-scroll-out" v-bind:class="{'pull-up': y > 0, 'refreshing': y > range/2}" v-if="top">
...
export default {
  props: {
    ...
    range: {
      // swipe range out of the eventArea
      type: Number,
      default: 100
    },
    freshtext: {
  ...
  data() {
    return {
      ...
      currentTranslate: !this.isx && this.top ? 44 : 0,
      x: 0,
      y: !this.isx && this.top ? -44 : 0,
      uiScrollStyle: {
  ...
  methods: {
    ...
    panendArea: function($event) {
      ...
      this.currentTranslate =
        this.currentTranslate < -range.max ? -range.max : this.currentTranslate
    ...
    scrollCount: function($event) {
      ...
      var addTranslate =
        this.currentTranslate + this[this.isEnd ? 'getDisByStep' : 'getDistance']($event)
    ...
    getRange: function(isCountCurrent) {
      ...
      var tophw =
        this.$el.querySelectorAll('.ui-scroll-top').length &&
        this.$el.querySelectorAll('.ui-scroll-top')[0][chw]
      var endhw =
        this.$el.querySelectorAll('.ui-scroll-end').length &&
        this.$el.querySelectorAll('.ui-scroll-end')[0][chw]
      return {
        min: range,
        max: tophw * 2 + endhw + range + corehw - fullhw
      }
    },
